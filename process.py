...
This is a set of utility functions I wrote during the process of exploring,
analyzing, and visualizing Fare Evasion Arrests in New York City
...
def gpd_from_csv(filename, crs={'init':'epsg:4326'}, long='Longitude', lat='Latitude'):
    '''
        ...
        Reads and returns a GeoDataFrame with geometry from
        "lat/long" columns (really can be any xy coordinates)
    '''
...
def populate_arrests(stations,
    ...
    '''
        Populate the subway station DataFrame with the number of arrests
        at each station according to the evasions DataFrame
    ...
    ev_dates = evasions[evasions['ARREST_DATE'] >= start_date]
    ev_dates = ev_dates[evasions['ARREST_DATE'] <= end_date]

    st_copy = stations.copy()

    if arr_column in stations.columns:
        st_copy = st_copy.drop(arr_column, axis=1)

    if 'complex_id' in st_copy.columns:
        st_copy = st_copy.set_index('complex_id')

    st_copy[arr_column] = ev_dates.groupby('station_id').count()['ARREST_DATE']

    st_copy[arr_column] = st_copy[arr_column].fillna(value=0)
    ...
    if breakdown_race:
        ...
        st_copy[race_arrests_station.columns] = race_arrests_station

        if combine_hispanic:
            try:
                st_copy['Hispanic'] = st_copy['Black Hispanic'] + st_copy['White Hispanic']

            except KeyError:
                if 'White Hispanic' in st_copy.columns:
                    st_copy['Hispanic'] = st_copy['White Hispanic']

                elif 'Black Hispanic' in st_copy.columns:
                    st_copy['Hispanic'] = st_copy['Black Hispanic']
                else:
                    st_copy['Hispanic'] = 0

            finally:
                st_copy['White Hispanic'] = 0
                st_copy['Black Hispanic'] = 0

            st_copy = st_copy.drop(['Black Hispanic', 'White Hispanic'], axis=1)

        st_copy = st_copy.fillna(0)
...
def get_intersecting_fips(station_buffer_row, census):
    ...
    for fips, tract in census.iterrows():
        ...
        if st_buff.intersects(census_geom):
            ...
            buff_union = st_buff.intersection(census_geom)
            weight = buff_union.area / st_area
...
def get_station_arrests(evasions, stations, subway_radius):
    ...
    #         print(nearest_idx)
            complex_nm = stations.loc[nearest_idx]['complex_nm']
